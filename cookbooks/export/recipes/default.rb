...
basedir = "/srv/export"
planet = node[:accounts][:system][:osrm][:home] + "/osmdata/osmdata.pbf"
...
git "#{basedir}/software/libosmium" do
  action :sync
...
git "#{basedir}/software/osmium-tool" do
  action :sync
...
directory "#{basedir}/software/osmium-tool/build" do
  owner "export"
  group "export"
  mode 0o755
end
...
execute "compile-osmium-tool" do
  action :nothing
  command "cmake .. && make"
  cwd "#{basedir}/software/osmium-tool/build"
  user "export"
  group "export"
  subscribes :run, "git[#{basedir}/software/osmium-tool]"
  subscribes :run, "git[#{basedir}/software/libosmium]"
  subscribes :run, "git[#{basedir}/software/protozero]"
  subscribes :run, "directory[#{basedir}/software/osmium-tool/build]"
end
